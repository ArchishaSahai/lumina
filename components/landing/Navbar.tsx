...
export function Navbar() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6">
    <nav className="mx-auto flex max-w-6xl items-center justify-between rounded-2xl border border-white/[.1] bg-black/55 px-4 py-3 backdrop-blur-xl sm:px-5">
      ...
      <div className="hidden items-center gap-2 sm:flex"><Button variant="ghost" onClick={() => router.push("/sign-in")} className="text-zinc-300 hover:bg-white/[.07] hover:text-white">Log in</Button><Button onClick={getStarted} className="bg-violet-500 px-4 text-white shadow-[0_0_24px_rgba(139,92,246,.3)] hover:bg-violet-400">Get started</Button></div>
    ...
    {open && <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mx-auto mt-2 max-w-6xl rounded-2xl border border-white/[.1] bg-zinc-950/95 p-3 backdrop-blur-xl md:hidden">{links.map((link) => <a onClick={() => setOpen(false)} className="block cursor-pointer rounded-lg px-3 py-2.5 text-sm text-zinc-300 outline-none transition-colors hover:bg-white/[.06] hover:text-white focus-visible:ring-2 focus-visible:ring-violet-400/70" href={link.href} key={link.label}>{link.label}</a>)}<div className="mt-2 grid gap-2 border-t border-white/[.08] pt-2 sm:hidden"><Button variant="ghost" onClick={() => router.push("/sign-in")} className="justify-start text-zinc-300">Log in</Button><Button onClick={getStarted} className="bg-violet-500 text-white hover:bg-violet-400">Get started</Button></div></motion.div>}
  </header>;
}
