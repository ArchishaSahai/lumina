...
export function ContextPanel({ notebookId, title, description, sources, updatedAt }: Props) {
  ...
  return (
    <aside className="min-h-0 overflow-y-auto border-t border-white/[.08] bg-zinc-950/65 p-4 xl:border-t-0 xl:border-l">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="mt-5 flex flex-wrap gap-2 text-xs text-zinc-500"><span className="inline-flex items-center gap-1.5 rounded-full border border-white/[.08] px-2.5 py-1"><Files className="size-3.5 text-violet-300" /> {sourceList.length} sources</span><span className="inline-flex items-center gap-1.5 rounded-full border border-white/[.08] px-2.5 py-1"><Clock3 className="size-3.5" /> {updatedAt}</span></div>
      </motion.div>
