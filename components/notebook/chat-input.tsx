...
export function ChatInput({ input, onChange, onSubmit, onStop, isGenerating, onKeyDown }: Props) {
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-white/[.1] bg-zinc-950/85 p-2 shadow-[0_-12px_40px_rgba(0,0,0,.2)] backdrop-blur-xl">
      <textarea value={input} onChange={onChange} onKeyDown={onKeyDown} className="min-h-20 w-full resize-none bg-transparent px-3 pt-2 text-sm text-white outline-none placeholder:text-zinc-600" placeholder="Ask anything about this notebook..." aria-label="Ask a question" />
