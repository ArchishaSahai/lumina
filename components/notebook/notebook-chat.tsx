...
export function NotebookChat({ notebookId, conversationId, messages, hasSources, onConversationCreated, onConversationTitleChanged, onConversationUpdated, onCitationView, onRefresh }: Props) {
  ...
  return <main className="flex min-h-[580px] min-w-0 flex-col bg-black/35"><div className="flex items-center justify-between border-b border-white/[.07] px-5 py-4"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-violet-300">Chat</p><p className="mt-1 text-sm text-zinc-500">{status === "searching" ? "Searching sources..." : status === "generating" ? "Generating answer..." : hasSources ? "Grounded in your notebook sources" : "Add sources to ground your responses"}</p></div><span className="rounded-full border border-violet-300/15 bg-violet-400/[.08] px-2.5 py-1 text-xs text-violet-200">{isEmpty ? "Ready" : "In context"}</span></div><div ref={viewportRef} onScroll={() => setAutoScroll(isAtBottom(viewportRef.current))} className="flex-1 overflow-y-auto px-5 py-8 sm:px-8">{isEmpty ? <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mx-auto flex max-w-lg flex-col items-center py-16 text-center"><div className="grid size-16 place-items-center rounded-2xl border border-violet-300/15 bg-violet-400/[.1] text-violet-300 shadow-[0_0_40px_rgba(139,92,246,.16)]"><Sparkles className="size-7" /></div><h2 className="mt-6 font-heading text-2xl font-semibold tracking-[-.04em]">{hasSources ? "What would you like to explore?" : "Add sources to begin exploring"}</h2><p className="mt-3 text-sm leading-6 text-zinc-400">{hasSources ? "Ask a question, compare ideas across your sources, or start outlining your next insight." : "Upload a document, video, or note from the source panel to give this conversation context."}</p></motion.section> : <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }} className="mx-auto flex max-w-3xl flex-col gap-7">{liveMessages.map((message) => <ChatBubble key={message.id} message={message} expandedCitations={selectedCitations} onToggleCitation={toggleCitation} onCitationHover={(citation) => onCitationView(citation.sourceId, true)} onCopy={message.role === "assistant" ? async () => { await navigator.clipboard.writeText(message.content); toast.success("Copied response"); } : undefined} onRegenerate={message.role === "assistant" ? () => handleRegenerate(message) : undefined} />)}{status !== "idle" && <TypingIndicator />}</motion.div>}</div><div className="px-5 pb-5 sm:px-8 sm:pb-7"><div className="mx-auto max-w-3xl"><ChatInput input={input} onChange={(event) => setInput(event.target.value)} onSubmit={(event) => { event.preventDefault(); void submitQuestion(); }} onStop={() => abortRef.current?.abort()} isGenerating={status !== "idle"} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submitQuestion(); } }} /><p className="mt-3 text-center text-[11px] text-zinc-600">Lumina can make mistakes. Review important details against your sources.</p></div></div></main>;
}
