...
import { Check, FileCode2, FileText, Film, Globe2, LoaderCircle, MoreHorizontal, Pencil, RefreshCw, Trash2 } from "lucide-react";
import { toast } from "sonner";
import { deleteSource, renameSource, retrySource } from "@/app/actions/sources";
...
export function SourceCard({ source, index, highlighted = false, onDeleted, onRenamed }: Props) {
  ...
  const rename = () => startTransition(async () => {
    ...
    if (!trimmed) return;
    try {
      const updated = await renameSource(source.id, trimmed);
      onRenamed?.(updated);
      setRenameOpen(false);
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-zinc-200">{source.title}</span>
            <span className="mt-1 flex items-center gap-1.5 text-[11px] text-zinc-500">
              <span className={`rounded-full px-1.5 py-0.5 font-medium ${typeStyles}`}>{typeLabel}</span>
              <span>·</span>
              <span className={source.status === "FAILED" ? "text-rose-300" : source.status === "READY" ? "text-emerald-300" : "text-sky-300"}>{statusLabel}</span>
            </span>
          </span>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" onClick={(event) => event.stopPropagation()} className="border border-white/[.1] bg-zinc-950 p-1 text-white shadow-xl">
              <DropdownMenuItem
                ...
                className="cursor-pointer text-zinc-300 focus:bg-white/[.08] focus:text-white"
              >
        ...
        {source.status === "PROCESSING" && (
          <div className="mt-3 flex items-center gap-2 text-[11px] text-sky-200">
            <LoaderCircle className="size-3 animate-spin" /> {stages[stageIndex]}
          </div>
        )}
      </motion.article>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="border-white/[.12] bg-zinc-950 p-6 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{source.status === "FAILED" ? "Processing failed" : "Processing source"}</DialogTitle>
            <DialogDescription className="truncate text-zinc-400">{source.title}</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            {stages.map((stage, itemIndex) => {
              const done = source.status === "READY" || itemIndex < stageIndex;
              const active = source.status === "PROCESSING" && itemIndex === stageIndex;
              return (
                <div key={stage} className="flex items-center gap-3 rounded-lg bg-white/[.03] px-3 py-2 text-sm">
                  <span className={done ? "text-emerald-300" : active ? "text-sky-300" : "text-zinc-600"}>
                    {done ? <Check className="size-4" /> : active ? <LoaderCircle className="size-4 animate-spin" /> : <span className="block size-4 rounded-full border border-current" />}
                  </span>
                  <span className={done || active ? "text-zinc-200" : "text-zinc-500"}>{stage}</span>
                </div>
              );
            })}
          </div>
          {source.status === "FAILED" && (
            <p className="text-sm text-rose-300">{source.processingError?.replace(/^stage:[A-Z]+\s*/, "") || "Unable to process this source."}</p>
          )}
          <DialogFooter className="-mx-6 -mb-6 rounded-b-2xl border-t border-white/[.08] bg-zinc-950 p-4">
            <Button type="button" onClick={retry} disabled={isPending} className="bg-violet-500 text-white hover:bg-violet-400">
      ...
      <Dialog open={renameOpen} onOpenChange={(open) => { if (!isPending) setRenameOpen(open); }}>
        <DialogContent className="border-white/[.12] bg-zinc-950 p-6 text-white sm:max-w-md">
          ...
          <Input
            ...
            className="h-10 border-white/[.12] bg-white/[.04] text-white"
            autoFocus
            maxLength={160}
          />
          <DialogFooter className="-mx-6 -mb-6 rounded-b-2xl border-t border-white/[.08] bg-zinc-950 p-4">
            <Button type="button" variant="ghost" onClick={() => setRenameOpen(false)} disabled={isPending} className="text-zinc-300 hover:bg-white/[.08] hover:text-white">
              Cancel
            </Button>
            <Button type="button" onClick={rename} disabled={!title.trim() || isPending} className="bg-violet-500 text-white hover:bg-violet-400">
        ...
        <DialogContent className="border-white/[.12] bg-zinc-950 p-6 text-white sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="leading-6 text-zinc-400">
              This will permanently delete <span className="font-medium text-zinc-200">{source.title}</span> and its stored file. This cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="-mx-6 -mb-6 rounded-b-2xl border-t border-white/[.08] bg-zinc-950 p-4">
            <Button type="button" variant="ghost" onClick={() => setDeleteOpen(false)}>Cancel</Button>
            <Button type="button" variant="destructive" onClick={remove} disabled={isPending}>{isPending ? "Deleting..." : "Delete source"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
