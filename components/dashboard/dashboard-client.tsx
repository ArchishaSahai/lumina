...
function DashboardSection({ notebooks, onRename, onDelete }: { notebooks: Notebook[]; onRename: (id: string, title: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  return <section className="mt-14"><h2 className="font-heading text-xl font-medium tracking-[-.03em]">Recent notebooks</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{notebooks.map((notebook) => <NotebookCard key={notebook.id} notebook={notebook} onRename={onRename} onDelete={onDelete} />)}</div></section>;
}

function EmptyState({ onCreate }: { onCreate: () => void }) {
  return <section className="mx-auto flex max-w-xl flex-col items-center py-20 text-center sm:py-28"><div className="grid size-24 place-items-center rounded-[2rem] border border-violet-300/15 bg-violet-400/[.1] text-violet-300 shadow-[0_0_56px_rgba(139,92,246,.2)]"><BookOpen className="size-11" strokeWidth={1.5} /></div><h2 className="mt-8 font-heading text-2xl font-medium tracking-[-.04em]">Create your first notebook</h2><p className="mt-3 max-w-md text-sm leading-6 text-zinc-400">Bring your sources, ideas, and notes together in one focused workspace.</p><Button size="lg" onClick={onCreate} className="mt-8 bg-violet-500 text-white shadow-[0_0_28px_rgba(139,92,246,.28)] hover:bg-violet-400"><Plus className="size-4" /> Create notebook</Button></section>;
}
